...
export async function GET(req: NextRequest, { params }: { params: { id: string | undefined | null } }) {
  try {
    ...
    const res = await fetch(`${process.env.MONGODB_SERVICE_URL}/api/issue/${id}`, {
      method: 'GET',
      cache: 'no-store',
      headers: {
    ...
    if (!res.ok) {
      return NextResponse.json({ error: 'Failed to get issue' }, { status: res.status });
    }

    const issue: Issue | null = await res.json();
...
export async function PUT(req: NextRequest, { params }: { params: { id: string | undefined | null } }) {
  try {
    ...
    const res = await fetchWithEmailHeader(`${process.env.MONGODB_SERVICE_URL}/api/issue/${id}`, {
      method: 'PUT',
      headers: {
        accept: 'application/json',
